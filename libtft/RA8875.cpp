...
namespace hw
{
	...
	bool RA8875::begin(TFT_DisplaySize s)
	{
		m_size = s;
		if (m_size == _480x272)
		{
		...
		uint8_t x = readRegister8(TFT_Register::STSR);
	...
	{
		if (on)
			setRegister8(TFT_Register::PWRR, RA8875_PWRR_NORMAL | RA8875_PWRR_DISPON);
		else
			setRegister8(TFT_Register::PWRR, RA8875_PWRR_NORMAL | RA8875_PWRR_DISPOFF);
	}
	...
	void RA8875::sleep(bool sleep) const
	{
		if (sleep) {
			GPIOX(false);
			setRegister8(TFT_Register::PWRR, RA8875_PWRR_DISPOFF | RA8875_PWRR_SLEEP);
		}
		else {
			setRegister8(TFT_Register::PWRR, RA8875_PWRR_DISPOFF);
			GPIOX(true);
		}
	}
	...
	{
		setRegister16(TFT_Register::CURH0, x);
		setRegister16(TFT_Register::CURV0, y);
	}
	...
	*/
	...
	void RA8875::drawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) const
	{
		...
		setRegister16(TFT_Register::DLVSR0, y0);
		setRegister16(TFT_Register::DLHER0, x1);
		setRegister16(TFT_Register::DLVER0, y1);
		setColorRegister(TFT_Register::FGCR0, color);

		setRegister8(TFT_Register::DCR, 0x80);
		waitPoll(TFT_Register::DCR, RA8875_DCR_LINESQUTRI_STATUS);
	}
	...
	{
		m_brightness = val;
		PWM1out(m_brightness);
	}

	void RA8875::backlight(bool on) const
	{
		GPIOX(on);
		if (on == true) {
			PWM1config(true, RA8875_PWM_CLK_DIV1024);//setup PWM ch 1 for backlight
			PWM1out(m_brightness); //turn on PWM1
		}
		else {
			PWM1config(false, RA8875_PWM_CLK_DIV1024);//setup PWM ch 1 for backlight
		}
	}
	...
	{
		if (on)
			setRegister8(TFT_Register::GPIOX, 1);
		else
			setRegister8(TFT_Register::GPIOX, 0);
	}

	void RA8875::PWM1config(bool on, uint8_t clock) const
	{
		if (on)
			setRegister8(TFT_Register::P1CR, RA8875_P1CR_ENABLE | (clock & 0xF));
		else
			setRegister8(TFT_Register::P1CR, RA8875_P1CR_DISABLE | (clock & 0xF));
	}

	void RA8875::PWM2config(bool on, uint8_t clock) const
	{
		if (on)
			setRegister8(TFT_Register::P2CR, RA8875_P2CR_ENABLE | (clock & 0xF));
		else
			setRegister8(TFT_Register::P2CR, RA8875_P2CR_DISABLE | (clock & 0xF));
	}

	void RA8875::PWM1out(uint8_t p) const
	{
		setRegister8(TFT_Register::P1DCR, p);
	}

	void RA8875::PWM2out(uint8_t p) const
	{
		setRegister8(TFT_Register::P2DCR, p);
	}
	...
	void RA8875::touchEnable(bool on) const
	{
		...
		if (on)
		{
			...
			setRegister8(TFT_Register::TPCR0, RA8875_TPCR0_ENABLE | RA8875_TPCR0_WAIT_4096CLK | RA8875_TPCR0_WAKEENABLE | adcClk); // 10mhz max!
			/* Set Auto Mode      (Reg 0x71) */
			setRegister8(TFT_Register::TPCR1, RA8875_TPCR1_AUTO | RA8875_TPCR1_DEBOUNCE);
			/* Enable TP INT */
			setRegister8(TFT_Register::INTC1, readRegister8(TFT_Register::INTC1) | RA8875_INTC1_TP);
		}
		else
		{
			/* Disable TP INT */
			...
			setRegister8(TFT_Register::TPCR0, RA8875_TPCR0_DISABLE);
		}
	}

	bool RA8875::touched() const
	{
		return (readRegister8(TFT_Register::INTC2) & RA8875_INTC2_TP) != 0;
	}
	...
	bool RA8875::touchRead(uint16_t *x, uint16_t *y) const
	{
		...
		tx = readRegister8(TFT_Register::TPXH);
		ty = readRegister8(TFT_Register::TPYH);
		temp = readRegister8(TFT_Register::TPXYL);
		tx <<= 2;
		ty <<= 2;
		...
		*y = ty;
		...
		/* Clear TP INT Status */
		setRegister8(TFT_Register::INTC2, RA8875_INTC2_TP);
		return true;
	}

	void RA8875::setRegister8(TFT_Register reg, uint8_t val) const
	{
		writeCommand(uint8_t(reg));
		writeData(val);
	}

	void RA8875::setRegister16(TFT_Register reg, uint16_t val) const
	{
	...
	void RA8875::setColorRegister(TFT_Register reg, uint16_t color) const
	{
		writeCommand(uint8_t(reg) + 0);
	...
	uint8_t RA8875::readRegister8(TFT_Register reg) const
	{
		writeCommand(uint8_t(reg));
		return readData();
	}
	...
	bool RA8875::waitPoll(TFT_Register reg, uint8_t f) const
	{
		while (1)
		{
	...
	void RA8875::PLLinit() const
	{
		if (m_size == _480x272)
		{
			setRegister8(TFT_Register::PLLC1, RA8875_PLLC1_PLLDIV1 + 10);
			delay(1);
			setRegister8(TFT_Register::PLLC2, RA8875_PLLC2_DIV4);
			delay(1);
		}
		else /* if (m_size == _800x480) */
		{
	...
	{
		setRegister8(TFT_Register::PLLC1, pll1);////PLL Control Register 1
		delay(1);
		setRegister8(TFT_Register::PLLC2, pll2);////PLL Control Register 2
		delay(1);
		setRegister8(TFT_Register::PCSR, pixclk);//Pixel Clock Setting Register
		delay(1);
	}
	...
	{
		PLLinit();
		setRegister8(TFT_Register::SYSR, RA8875_SYSR_16BPP | RA8875_SYSR_MCU8);
		...
		setRegister8(TFT_Register::PCSR, pixclk);
		delay(1);

		/* Horizontal settings registers */
		setRegister8(TFT_Register::HDWR, uint8_t((m_width / 8) - 1));                 // H width: (HDWR + 1) * 8 = 480
		setRegister8(TFT_Register::HNDFTR, RA8875_HNDFTR_DE_HIGH + hsync_finetune);
		setRegister8(TFT_Register::HNDR, (hsync_nondisp - hsync_finetune - 2) / 8);   // H non-display: HNDR * 8 + HNDFTR + 2 = 10
		setRegister8(TFT_Register::HSTR, hsync_start / 8 - 1);                        // Hsync start: (HSTR + 1)*8 
		setRegister8(TFT_Register::HPWR, RA8875_HPWR_LOW + (hsync_pw / 8 - 1));       // HSync pulse width = (HPWR+1) * 8

		/* Vertical settings registers */
		setRegister16(TFT_Register::VDHR0, m_height - 1);
		setRegister16(TFT_Register::VNDR0, vsync_nondisp - 1);                        // V non-display period = VNDR + 1
		...
		setRegister8(TFT_Register::MCLR, RA8875_MCLR_START | RA8875_MCLR_FULL);
		delay(500);
	}

	void RA8875::circleHelper(uint16_t x0, uint16_t y0, uint8_t r, uint16_t color, bool filled) const
	{
		setRegister16(TFT_Register::DCHR0, x0);
		setRegister16(TFT_Register::DCVR0, y0);
		setRegister8(TFT_Register::DCRR, r);
		setColorRegister(TFT_Register::FGCR0, color);

		/* Draw! */
		setRegister8(TFT_Register::DCR, RA8875_DCR_CIRCLE_START | (filled ? RA8875_DCR_FILL : RA8875_DCR_NOFILL));

		/* Wait for the command to finish */
		waitPoll(TFT_Register::DCR, RA8875_DCR_CIRCLE_STATUS);
	}

	void RA8875::rectHelper(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color, bool filled) const
	{
		setRegister16(TFT_Register::DLHSR0, x);
		setRegister16(TFT_Register::DLVSR0, y);
		setRegister16(TFT_Register::DLHER0, w);
		setRegister16(TFT_Register::DLVER0, h);
		setColorRegister(TFT_Register::FGCR0, color);

		/* Draw! */
		setRegister8(TFT_Register::DCR, filled ? 0xB0 : 0x90);
	...
	void RA8875::triangleHelper(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color, bool filled) const
	{
		...
		setRegister16(TFT_Register::DLHER0, x1);
		setRegister16(TFT_Register::DLVER0, y1);
		setRegister16(TFT_Register::DTPH0, x2);
		setRegister16(TFT_Register::DTPV0, y2);
		setColorRegister(TFT_Register::FGCR0, color);

		/* Draw! */
		setRegister8(TFT_Register::DCR, filled ? 0xA1 : 0x81);
	...
	{
		setRegister16(TFT_Register::DEHR0, xCenter);
		setRegister16(TFT_Register::DEVR0, yCenter);
		setRegister16(TFT_Register::ELL_A0, longAxis);
		setRegister16(TFT_Register::ELL_B0, shortAxis);
		setColorRegister(TFT_Register::FGCR0, color);

		/* Draw! */
		setRegister8(TFT_Register::ELLIPSE, filled ? 0xC0 : 0x80);

		/* Wait for the command to finish */
		waitPoll(TFT_Register::ELLIPSE, RA8875_ELLIPSE_STATUS);
	}
	...
	void RA8875::curveHelper(uint16_t xCenter, uint16_t yCenter, uint16_t longAxis, uint16_t shortAxis, uint8_t curvePart, uint16_t color, bool filled) const
	{
		...
		/* Draw! */
		setRegister8(TFT_Register::ELLIPSE, (filled ? 0xD0 : 0x90) | (curvePart & 0x03));
